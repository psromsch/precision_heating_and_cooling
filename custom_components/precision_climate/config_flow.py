...
class PrecisionClimateOptionsFlow(config_entries.OptionsFlow):
    ...
    async def _room_form(self, user_input: dict | None, defaults: dict | None = None):
        ...
        if user_input is not None:
            ...
            # silently merges two rooms in the UI), and a TRV shared by two
            # rooms makes the control loop command the same valve with two
            # conflicting decisions (perpetual flapping). Only ACTUATORS must
            # be exclusive — sharing a thermometer (or window sensor) across
            # rooms is read-only and perfectly legitimate (e.g. a hallway
            # reading a bedroom's sensor).
            ...
            other_trvs: set[str] = set()
            for r in other_rooms:
                other_trvs.update(r.get(CONF_TRVS, []))
            own_trvs = list(user_input[CONF_TRVS])
            ...
            elif any(e in other_trvs for e in own_trvs):
